 ...
 */
    BufferPoolManager::BufferPoolManager(size_t pool_size,
        ...
        // put all the map into free list
 ...
 */
    Page *BufferPoolManager::FetchPage(page_id_t page_id) {
        ...
        if (page_table_->Find(page_id, target)) {
            target->pin_count_++;
            replacer_->Erase(target);  //无法替换
            return target;
        }
        // 1.2 搜索，不存在，从空闲列表或lru替换器中查找
        //先空闲，后lru替换器
        if (!free_list_->empty()) {
 ...
 */
    bool BufferPoolManager::UnpinPage(page_id_t page_id, bool is_dirty) {
        ...
        // 减少pin计数器.如果pin计数器为零,把Page对象添加到LRUReplacer跟踪器中
        if (--target->pin_count_ == 0) replacer_->Insert(target);
 ...
 */
    bool BufferPoolManager::FlushPage(page_id_t page_id) {
        ...
        Page *target = nullptr;
        if (!page_table_->Find(page_id, target)) return false;
 ...
 * into page table. return nullptr if all the map in pool are pinned
 */
